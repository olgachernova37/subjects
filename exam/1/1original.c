...
int ft_popen(const char *file, char *const av[], char type)
{
    ...
    if (pid == 0)  // ДОЧІРНІЙ ПРОЦЕС
    {
        if (type == 'r')
        {
            // Для читання: перенаправляємо STDOUT дочірнього процесу на fd[1]
            // Батьківський процес буде читати з fd[0]
            dup2(fd[1], STDOUT_FILENO);
        }
        else  // type == 'w'
        {
            // Для запису: перенаправляємо STDIN дочірнього процесу на fd[0]
            // Батьківський процес буде писати в fd[1]
            dup2(fd[0], STDIN_FILENO);
        }
        
        // Закриваємо обидва дескриптори в дочірньому процесі
        // (вони більше не потрібні після dup2)
        close(fd[0]);
        close(fd[1]);
        
        // Замінюємо дочірній процес новою програмою
        execvp(file, av);
        
        // Якщо execvp повернувся, значить сталася помилка
        exit(1);
    }
    ...
    if (type == 'r')
    {
        // Для читання: закриваємо fd[1] (запис)
        // Повертаємо fd[0] для читання
        close(fd[1]);
        return (fd[0]);
    }
    else  // type == 'w'
    {
        // Для запису: закриваємо fd[0] (читання)
        // Повертаємо fd[1] для запису
        close(fd[0]);
        return (fd[1]);
    }
}
